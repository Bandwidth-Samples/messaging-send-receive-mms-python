...
bandwidth_media_api_instance = bandwidth.MediaApi(bandwidth_api_client)
...
def handle_inbound(data: List[bandwidth.models.InboundMessageCallback]):
    ...
    if len(data[0].message.media) > 0:
        for media in data[0].message.media:
            media_id = media.media_id.split('/media',1)[1]
            media_response = bandwidth_media_api_instance.get_media(BW_ACCOUNT_ID, media.media_id)

            with open(f"media_{media.media_id}.{media.content_type.split('/')[1]}", "wb") as f:
                f.write(media_response.data)

            print(f"Media saved to media_{media.media_id}.{media.content_type.split('/')[1]}")
